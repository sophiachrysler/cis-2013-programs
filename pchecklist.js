...
{
    var stringPassword, arrayBadPasswords, stringOutput, intPasswordCheck;
    
    
    stringPassword = $("password").value; //gets the password from the DOM input
...
if (stringPassword.length < 8){
      stringOutput = "Your password is not long enough";
      }
else{
      for (counter = 0; counter < arrayBadPasswords.length; counter ++){
       if(stringPassword === arrayBadPasswords[counter] ){
        intPasswordCheck = 1; //this will assign the value of 1 to the variable intPasswordCheck, which is evaluated in the next step
      }
      }
      //the next part of code evaluates the variable intPasswordCheck
 ...
 $("output").value = stringOutput; //sends stringOutput to the DOM
};
